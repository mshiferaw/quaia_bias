...
do 
    ...
    python make_catalogs.py ${G} --n_zbins ${n_zbins[j]} --n_Lbins ${n_Lbins[j]} --no-threshold
    python make_catalogs.py ${G} --n_zbins ${n_zbins[j]} --n_Lbins ${n_Lbins[j]} --threshold
    
    for ((zb=0; zb<${n_zbins[j]}; zb++))
    do
        ...
        do
            ...
            #     fname="_zsplit${n_zbins[j]}bin${zb}_Lmaxsplit${n_Lbins[j]}bin${lb}"
            # else
                fname="_zsplit${n_zbins[j]}bin${zb}${L}bin${lb}"
            # fi
