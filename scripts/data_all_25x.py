...
def main():
    ...
    tab_datahi_mask_zbin0, tab_randhi_mask_zbin0, key_zbin0, _, _ = quaia.make_bins(G, 0, True, 'z', tab_gcat_type = 'data', z = False, fac_rand = 25)
    ...
    np.save('../results/wtheta_G{:.1f}_25x'.format(G, zbin), wtheta_datahi_zbin0)
    ...
    np.save('../results/wp_G{:.1f}_25x'.format(G, zbin), wp_datahi_zbin0)
    np.save('../results/rpavg_G{:.1f}_25x'.format(G, zbin), rpavg_datahi_zbin0)
