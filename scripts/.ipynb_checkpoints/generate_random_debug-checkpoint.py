...
def main():
    ...
    fn_selfunc =  f'../data/maps/selection_function_NSIDE{NSIDE_map}_G{G_max}{fname}{selfunc}.fits'
    print(fn_selfunc)
    # fn_rand = f'../data/randoms/random_G{G_max}{fname}{allsky}_{fac_rand}x.fits'
    fn_rand = f'../data/randoms/random_G{G_max}{fname}{selfunc}_{fac_rand}x.fits'
    print(fn_rand)
    overwrite = True
...
def run(fn_selfunc, NSIDE_map, fn_rand, allsky, fn_catalog=None, fac_rand=1,
    ...
    # Load data; only used to figure out desired number
        ...
        _, pixel_indices_catalog = maps.get_map(NSIDE_map, tab_catalog['ra'], tab_catalog['dec'])
    ...
    ra_rand, _ = subsample_by_probmap(NSIDE_map, rng, ra_rand_try, dec_rand_try, fn_selfunc, allsky, pixel_indices_catalog)
    ...
    if allsky == '_allsky':
        ...
        ra_rand, dec_rand = subsample_by_probmap(NSIDE_map, rng, ra_rand_init, dec_rand_init, fn_selfunc, allsky, pixel_indices_catalog)
...
def indices_for_downsample(rng, probability_accept, allsky):
...
def subsample_by_probmap(NSIDE_map, rng, ra, dec, fn_selfunc, allsky, pixel_indices_catalog,
                         normalize=True):
    ...
    print(np.min(map_p), np.min(p_rand))
    if allsky == '_map_p': # test if this makes a difference
        p_rand /= np.max(map_p)
    else:
        p_rand /= np.max(p_rand)
    idx_keep = indices_for_downsample(rng, p_rand, allsky)

    print(f"Subsampling by {np.sum(idx_keep)/len(idx_keep):.3f} for probability map")
    # print('keep', idx_keep)
    # directly subsample somehow?? to ensure that zero remains zero
    if allsky == '_zero':

        # idx_zero = pixel_indices_catalog[map_p[pixel_indices_catalog]==0]
        print(np.min(map_p[pixel_indices_catalog]))
        NPIX = hp.nside2npix(NSIDE_map)
        idx_zero = set(np.arange(NPIX)).difference(set(pixel_indices_catalog)) # these are the indices where there are no quasars in the final dataset
        # now i need to find the indices where there are still quasars in the randoms, and remove them
        for i in range(len(pixel_indices_rand)):
            if pixel_indices_rand[i] in idx_zero:
                idx_keep[i]=False 
        print(np.where(map_p[pixel_indices_catalog]==0))
        # idx_keep = list(set(pixel_indices_rand).difference(set(idx_zero))) # convert to true/false by finding where the indices show up in the random catalog
        # idx_keep = list(set(idx_keep).difference(set(idx_zero)))
    print('keep', idx_keep)
    # print('zero', idx_zero)
    print(f"Subsampling by {np.sum(idx_keep)/len(idx_keep):.3f} for probability map")
