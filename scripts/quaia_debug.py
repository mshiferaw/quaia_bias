...
def comoving_dist(z, h = 0.6844, units = 'Mpc/h', Om0 = 0.302, Ob0 = None): # col 2 in fig 7 of https://arxiv.org/pdf/1807.06209
...
def z_dist(tab_gcatlo, tab_randlo, 
           # mask_gcatlo, mask_randlo,
           N_gcatlo_mask, N_randlo_mask, b = 0, mask_type = None, plot = False):
           # , new = False):
    ...
    z = np.sort(tab_gcatlo['redshift_quaia']) # already includes mask now
                # [mask_gcatlo])
    ...
    tab_randlo['redshift_quaia_'+key] = inv_ecdf(rng.random(size = N_randlo_mask))
    ...
    if plot == True:
        ...
        plt.figure()
        plt.plot(z, ecdf, label = 'ecdf', linewidth = 2.5)
        plt.hist(tab_gcatlo['redshift_quaia'],
                 # [mask_gcatlo], 
                 cumulative = True, density = True, bins = 100, label = 'data', histtype = 'step', linestyle = '--', linewidth = 2.5)
        plt.hist(tab_randlo['redshift_quaia_'+key], cumulative = True, density = True, bins = 100, label = 'randoms', histtype = 'step', linestyle = ':', linewidth = 2.5)
        plt.xlabel('$z$')
        plt.legend()
        plt.show()
...
def make_bins(G, bb, prebinned, cuts = None, method = None, tab_gcat = None, tab_gcat_type = None, i = None, n_bins = None, bins = None, z = True, fac_rand = 10, allsky = "", NSIDE = NSIDE, plot = False, mask_type = 'selfunc', b = 0, mask = 0.5, norm = None, percentile = False, scale = None, name_catalog = '$Gaia$-$unWISE$ Quasar Catalog', cmap_map = 'plasma', cbar_ticks = [5, 10, 20], fac_stdev = fac_stdev, bounds = [1, 16]):
    ...
    print(N_randhi_mask_bin0/N_datahi_mask_bin0)
    if z == True:
        key_bin0 = z_dist(tab_datahi_mask_bin0, tab_randhi_mask_bin0, 
                          # np.full(N_datahi_mask_bin0, True), 
                             # np.full(N_randhi_mask_bin0, True), 
                          N_datahi_mask_bin0, N_randhi_mask_bin0, plot = plot, 
    ...
    if plot == True:
        ...
        # make map of quasar number counts
        pixel_indices_gcatlo = hp.ang2pix(NSIDE, tab_datahi_mask_bin0['ra'], tab_datahi_mask_bin0['dec'], lonlat=True)
        NPIX = hp.nside2npix(NSIDE)
        map_gcatlo = np.bincount(pixel_indices_gcatlo, minlength=NPIX)
        print(np.min(map_gcatlo*hp.pixelfunc.nside2pixarea(NSIDE, degrees = True)))#-fac_stdev*np.std(map_gcatlo), np.mean(map_gcatlo)+fac_stdev*np.std(map_gcatlo))
...
def wp_rp(tab_gcatlo, tab_randlo, key, 
          # selfunc_lo = None, weights1 = None, weights2 = None, RR_counts = None, 
          nthreads = 8, 
