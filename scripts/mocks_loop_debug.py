...
def main():
    ...
    tab_datahi_mask_zbin0, tab_randhi_mask_zbin0, key_zbin0, _, _, _, _ = quaia.make_bins(G, zbin, False, 'z', tab_gcat_type = 'mocks', method = 'split', n_bins = 2, fac_rand = fac_rand, i = i+1, mask_type = None, b = b)
    ...
    np.save('../results/wp_G{:.1f}_zsplit2bin{}_b{}_mock{}_{}x'.format(G, zbin, b, i+1, fac_rand), wp_datahi_zbin0)
    np.save('../results/rpavg_G{:.1f}_zsplit2bin{}_b{}_mock{}_{}x'.format(G, zbin, b, i+1, fac_rand), rpavg_datahi_zbin0)
    ...
    np.save('../results/xi_G{:.1f}_zsplit2bin{}_b{}_mock{}_{}x'.format(G, zbin, b, i+1, fac_rand), cf_datahi_zbin0)
    ...
    cf_datahi_zbin0 = quaia.xi_s(tab_datahi_mask_zbin0, tab_randhi_mask_zbin0, key_zbin0, nthreads = nthreads, rbins = sbins)
    np.save('../results/xi_G{:.1f}_b{}_mock{}_{}x'.format(G, b, i+1, fac_rand), cf_datahi_zbin0)
    
    wp_datahi_zbin0, rpavg_datahi_zbin0 = quaia.wp_rp(tab_datahi_mask_zbin0, tab_randhi_mask_zbin0, key_zbin0, nthreads = nthreads, rbins = rbins,
                                                                   nbins = nbins, pimax = pimax)
    np.save('../results/wp_G{:.1f}_b{}_mock{}_{}x'.format(G, b, i+1, fac_rand), wp_datahi_zbin0)
    np.save('../results/rpavg_G{:.1f}_b{}_mock{}_{}x'.format(G, b, i+1, fac_rand), rpavg_datahi_zbin0)
